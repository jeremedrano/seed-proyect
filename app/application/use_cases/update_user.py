...
class UpdateUserUseCase:
    ...
    def execute(
        ...
        if user_id <= 0:
            LOG.warning(
                "Use case: UpdateUser - VALIDATION FAILED: User ID must be positive: %d", user_id
            )
        ...
        if email is not None:
            ...
            if not re.match(email_pattern, email):
                LOG.warning(
                    "Use case: UpdateUser - VALIDATION FAILED: Invalid email format: %s", email
                )
            ...
            if user_with_email and user_with_email.id != user_id:
                LOG.warning(
                    "Use case: UpdateUser - VALIDATION FAILED: Email already exists for another user"
                )
        ...
        if age is not None:
            ...
            if age <= 0:
                LOG.warning(
                    "Use case: UpdateUser - VALIDATION FAILED: Age must be positive: %d", age
                )
        ...
        updated_user = User(id=user_id, email=updated_email, name=updated_name, age=updated_age)
