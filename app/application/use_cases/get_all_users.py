...
class GetAllUsersUseCase:
    ...
    def execute(self, skip: int = 0, limit: int = 100) -> List[User]:
        ...
        if skip < 0:
            LOG.warning("Use case: GetAllUsers - VALIDATION FAILED: Skip must be non-negative: %d", skip)
        ...
        if limit <= 0:
            LOG.warning("Use case: GetAllUsers - VALIDATION FAILED: Limit must be positive: %d", limit)
        ...
        if limit > 100:
            LOG.warning("Use case: GetAllUsers - VALIDATION FAILED: Limit cannot exceed 100: %d", limit)
